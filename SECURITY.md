...
Virga parses JSON it did not write, from Open-Meteo and from ipapi.co, asks
GitHub which release is newest, writes files into your per-user state
directory, and ships an install script that people pipe into `sh`. Those
are the places a report is most likely to be about.
...
gets it; the README's [Updating and removing](README.md#updating-and-removing)
section has the command for each kind of install.

## What counts

Anything that makes Virga do something the user did not ask for: a response
from any host it talks to that crashes or hangs the app, a path escaping the
state directory, a file written unsafely, a way for the install script to
leave a bad binary on `PATH` or to run something it downloaded without
checking it.
