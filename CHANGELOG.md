...
- A save that cannot take its lock, the state file's or the forecast
  ...
  advisory locks at all may save unserialized; an I/O error, a kernel out
  of locks, or a permission refused used to be waved through the same
  way, and two writers could then race silently, which is the one thing
  the lock is for (#83).
